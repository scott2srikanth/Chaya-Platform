...
import type { AnimationClip } from "./clip-engine";
...
export type CharacterGesture =
  "idle" | "talking" | "pointing" | "waving" | "thinking" | "celebrating";
...
export interface Actor3D {
  animationPoses?: Record<string, {pose?: string; joints?: import("./pose3d").JointPose}>;
  jointPose?: import("./pose3d").JointPose;
  savedPoses?: import("./pose3d").SavedPose3D[];
  y?: number;
  scale?: number;
  pose?: string;
  motion?: string;
  ...
  motionEnd?: number;
  motionLoop?: boolean;
  travel?: { x: number; z: number; start: number; duration: number };
  manualPose?: boolean;
  x: number;
  z: number;
  rotation: number;
  ...
  hair: string;
  faceTexture?: string;
  appearance?: import("./appearance").CharacterAppearance;
  characterModel?: "alex-v1" | "sarah-v1" | "procedural";
  modelUrl?: string;
  modelName?: string;
  animation?: string;
}
export interface SceneElement {
  whiteboard?: import("./whiteboard").WhiteboardContent;
  actor3d?: Actor3D;
  id: string;
  type: ElementType;
  name: string;
  ...
  startTime?: number;
  endTime?: number;
  textEffect?: "none" | "typewriter" | "words" | "lines";
  ...
  appearanceOverride?: Partial<import("./rig").CharacterAppearance>;
  actions?: CharacterAction[];
  expressionKeys?: { time: number; expression: string }[];
...
export interface ScriptLine {
  ...
  performance?: import("./performance").SpeechPerformance;
  duration: number;
}
...
export interface Scene {
  room3d?: import("./ai-schema").Room3D;
  ...
  cameraMode?: "keyframes" | "dialogue";
  subtitles?: boolean;
  stage3d?: boolean;
  stageSet?: "living" | "interview";
}
...
export interface Project {
  ...
  clips?: AnimationClip[];
  templates?: { id: string; name: string; scene: Scene }[];
}
...
export function createElement(
  ...
  switch (type) {
    case "circle":
      ...
      overrides.fill = "#10b981";
      break;
    case "text":
      overrides.width = 400;
      overrides.height = 80;
      overrides.fill = "transparent";
      overrides.stroke = "transparent";
      overrides.text = "Your text here";
      break;
    case "line":
      overrides.width = 300;
      overrides.height = 0;
      overrides.fill = "transparent";
      overrides.stroke = "#334155";
      overrides.strokeWidth = 3;
      overrides.x2 = 300;
      overrides.y2 = 0;
      break;
    case "character":
      overrides.width = 240;
      overrides.height = 400;
      overrides.fill = "transparent";
      overrides.stroke = "transparent";
      overrides.characterGesture = "idle";
      overrides.characterReaction = "none";
      overrides.facingRight = true;
      break;
  }
...
const INTERVIEW_PROPS: TemplateProp[] = [
  ...
  bg("Panel L", 40, 20, 420, 680, "#172035", {
    stroke: "#1e2d4a",
    strokeWidth: 1,
    borderRadius: 8,
  }),
  bg("Panel R", 1460, 20, 420, 680, "#172035", {
    stroke: "#1e2d4a",
    strokeWidth: 1,
    borderRadius: 8,
  }),
  ...
  bg("Glow R", 1200, 0, 300, 50, "#f59e0b", {
    borderRadius: 25,
    opacity: 0.08,
  }),
  // Armchair backs (behind characters)
  bg("Chair L back", 410, 240, 210, 280, "#1c1828", {
    stroke: "#2a2640",
    strokeWidth: 2,
    borderRadius: 22,
  }),
  bg("Chair L top", 430, 230, 170, 20, "#1c1828", { borderRadius: 10 }),
  bg("Chair R back", 1310, 240, 210, 280, "#1c1828", {
    stroke: "#2a2640",
    strokeWidth: 2,
    borderRadius: 22,
  }),
  ...
  fg("Desk surface", 320, 468, 1280, 24, "#5c3d1e", {
    stroke: "#7a5228",
    strokeWidth: 2,
    borderRadius: 8,
  }),
...
const PRODUCT_DEMO_PROPS: TemplateProp[] = [
  ...
  // Screen mount wall
  bg("Screen wall", 560, 30, 800, 460, "#111827", {
    stroke: "#1f2937",
    strokeWidth: 2,
    borderRadius: 10,
  }),
  // Screen frame
  bg("Screen frame", 590, 50, 740, 400, "#1e293b", {
    stroke: "#334155",
    strokeWidth: 3,
    borderRadius: 8,
  }),
  // Screen glow border
  bg("Screen glow", 600, 60, 720, 380, "#0ea5e9", {
    borderRadius: 6,
    opacity: 0.15,
  }),
  // Screen interior
  ...
  bg("Content 1", 660, 150, 300, 6, "#0ea5e9", {
    borderRadius: 3,
    opacity: 0.3,
  }),
  bg("Content 2", 660, 180, 220, 6, "#0ea5e9", {
    borderRadius: 3,
    opacity: 0.2,
  }),
  bg("Content 3", 660, 210, 260, 6, "#0ea5e9", {
    borderRadius: 3,
    opacity: 0.25,
  }),
  // Screen stand
...
const CLASSROOM_PROPS: TemplateProp[] = [
  ...
  // Whiteboard area
  bg("WB border", 100, 50, 560, 400, "#a1a1aa", {
    stroke: "#71717a",
    strokeWidth: 3,
    borderRadius: 4,
  }),
  ...
  bg("Chalk 3", 160, 190, 240, 5, "#10b981", {
    borderRadius: 3,
    opacity: 0.35,
  }),
  // Clock on wall
  bg("Clock", 1700, 60, 70, 70, "#fafafa", {
    stroke: "#a1a1aa",
    strokeWidth: 2,
    borderRadius: 35,
  }),
  // Student chair (behind student)
  bg("Chair back", 1310, 410, 160, 200, "#ea580c", {
    stroke: "#c2410c",
    strokeWidth: 2,
    borderRadius: 12,
  }),
  bg("Chair seat", 1290, 610, 200, 22, "#ea580c", {
    stroke: "#c2410c",
    strokeWidth: 2,
    borderRadius: 8,
  }),
  ...
  fg("Desk top", 1180, 530, 400, 22, "#b45309", {
    stroke: "#92400e",
    strokeWidth: 2,
    borderRadius: 4,
  }),
...
export const SCRIPT_TEMPLATES: ScriptTemplate[] = [
  {
    ...
    lines: [
      {
        text: "Hey! Let me show you something incredible we've been working on.",
        gesture: "waving",
        reaction: "listening",
        shot: "medium",
      },
      {
        text: "Oh really? I'm all ears! What is it?",
        gesture: "talking",
        reaction: "nodding",
        shot: "medium",
      },
      {
        text: "It's a brand new way to create animated videos, right in your browser!",
        gesture: "pointing",
        reaction: "surprised",
        shot: "closeup",
      },
      {
        text: "Wow, that sounds amazing! How does it work?",
        gesture: "talking",
        reaction: "nodding",
        shot: "medium",
      },
      {
        text: "You just add characters, write your script, and hit play. It's that simple!",
        gesture: "celebrating",
        reaction: "laughing",
        shot: "wide",
      },
  ...
  },
  {
    ...
    lines: [
      {
        text: "Welcome to the show! Tell us about yourself.",
        gesture: "talking",
        reaction: "listening",
        shot: "medium",
      },
      ...
      {
        text: "So what inspired you to get started in this field?",
        gesture: "thinking",
        reaction: "listening",
        shot: "closeup",
      },
      {
        text: "It all started when I discovered how powerful technology can be for education.",
        gesture: "pointing",
        reaction: "nodding",
        shot: "medium",
      },
      {
        text: "That's fascinating! And where do you see things heading?",
        gesture: "talking",
        reaction: "thinking",
        shot: "medium",
      },
      {
        text: "I believe the future is all about making complex ideas simple and visual.",
        gesture: "celebrating",
        reaction: "laughing",
        shot: "wide",
      },
  ...
  },
  {
    ...
    lines: [
      {
        text: "Good morning class! Today we're going to learn something exciting.",
        gesture: "waving",
        reaction: "listening",
        shot: "medium",
      },
      {
        text: "Let me explain the key concept. Pay close attention to this.",
        gesture: "pointing",
        reaction: "nodding",
        shot: "closeup",
      },
      {
        text: "Excuse me, could you explain that part again?",
        gesture: "talking",
        reaction: "listening",
        shot: "medium",
      },
      {
        text: "Of course! Think of it like building blocks. Each piece connects to the next.",
        gesture: "talking",
        reaction: "thinking",
        shot: "medium",
      },
      {
        text: "Oh, I think I understand now! That makes so much sense!",
        gesture: "celebrating",
        reaction: "nodding",
        shot: "medium",
      },
      {
        text: "Excellent! You've got it. Now let's practice together.",
        gesture: "celebrating",
        reaction: "laughing",
        shot: "wide",
      },
...
export interface CameraKey extends CameraState {
  id: string;
  time: number;
  easing: EasingType;
}
...
export interface StudioAsset {
  id: string;
  name: string;
  type: "image" | "svg" | "sprite" | "lottie" | "rive" | "audio";
  dataUrl: string;
  width: number;
  height: number;
  duration?: number;
  sprite?: {
    ...
    rows: number;
    fps: number;
    animations: { name: string; from: number; to: number }[];
  };
  lottie?: Record<string, any>;
  rive?: {
    animations: string[];
    stateMachines: { name: string; inputs: { name: string; type: string }[] }[];
  };
  waveform?: number[];
}
