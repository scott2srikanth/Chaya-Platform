...
export function createKeyframe(time: number, poseId: string, easing: EasingType = 'easeInOut'): Keyframe {
  return { id: crypto.randomUUID(), time, poseId, easing };
}

export function createClip(elementId: string, name: string, duration: number, keyframes: Keyframe[] = []): AnimationClip {
  return {
...
export function insertKeyframe(clip: AnimationClip, kf: Keyframe): AnimationClip {
  const keyframes = [...clip.keyframes, kf].sort((a, b) => a.time - b.time);
  return { ...clip, keyframes };
}

export function removeKeyframe(clip: AnimationClip, keyframeId: string): AnimationClip {
  return { ...clip, keyframes: clip.keyframes.filter((k) => k.id !== keyframeId) };
}

export function updateKeyframe(clip: AnimationClip, keyframeId: string, updates: Partial<Keyframe>): AnimationClip {
...
export function evaluateClip(
  ...
  if (keyframes.length === 1) {
    return resolvePoseTransforms(character, keyframes[0].poseId);
  }
  ...
  if (t >= last.time) {
    if (loop && duration > 0) {
      ...
      if (segDuration <= 0) return resolvePoseTransforms(character, last.poseId);
      const elapsed = t - last.time;
      const raw = elapsed / segDuration;
      const eased = applyEasing(raw, keyframes[0].easing);
      const fromPose = resolvePoseTransforms(character, last.poseId);
      const toPose = resolvePoseTransforms(character, keyframes[0].poseId);
      return interpolatePoseTransforms(fromPose, toPose, eased);
    }
    return resolvePoseTransforms(character, last.poseId);
  }
  ...
  for (let i = 0; i < keyframes.length - 1; i++) {
    ...
    if (t >= kfA.time && t <= kfB.time) {
      ...
      if (segDuration <= 0) return resolvePoseTransforms(character, kfB.poseId);
      const raw = (t - kfA.time) / segDuration;
      const eased = applyEasing(raw, kfB.easing);
      const fromPose = resolvePoseTransforms(character, kfA.poseId);
      const toPose = resolvePoseTransforms(character, kfB.poseId);
      return interpolatePoseTransforms(fromPose, toPose, eased);
    }
  }
...
export function createConversationClip(elementId: string, role: 'speaker' | 'listener'): AnimationClip {
  if (role === 'speaker') {
...
export const CLIP_PRESETS = [
  ...
  { id: 'speaker', name: 'Speaker', create: (eid: string) => createConversationClip(eid, 'speaker') },
  { id: 'listener', name: 'Listener', create: (eid: string) => createConversationClip(eid, 'listener') },
] as const;
