...
import { interviewProject } from "../../lib/studio/templates";
...
async function main() {
  ...
  await writeFile("artifacts/studio/seated-interview.json", JSON.stringify(project));
  await mkdir("public/studio/examples", {recursive: true});
  await writeFile("public/studio/examples/seated-interview.json", JSON.stringify(project));
  console.log("Generated voiced seated interview:", project.scenes.map(s => s.duration));
}
